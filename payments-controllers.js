...
const createInitialPaymentIntent = async (req, res, next) => {
  try {
    const { opaqueData, addressLine1, addressLine2, city, postalCode, state } =
      req.body;
    ...
    if (!user.authorizeNetCustomerId) {
      const customerProfileId = await authorizeNet.createCustomerProfile(user);
    ...
    if (price.amount !== 0) {
      ({ transactionId, error } = await authorizeNet.chargeCustomerProfile({
        customerProfileId: user.authorizeNetCustomerId,
        price: price.amount,
        user,
      ...
      try {
        ...
        if (transactions) {
          for (const element of transactions.getTransaction()) {
            const transaction = await authorizeNet.getTransactionDetails(
              element.getTransId()
            );
            if (transaction.responseCode === 1) {
              paymentsList.push({
                description: transaction.getOrder().getDescription(),
        ...
        if (customerProfile.getPaymentProfiles().length > 0) {
          const paymentProfile = customerProfile.getPaymentProfiles()[0];
